...
import type { ReminderStore } from './store/reminderStore'
import { computeNextDueAtMs } from './utils/recurrence'

export type ReminderNotifier = {
  notifyReminderFired: (reminder: Reminder) => Promise<void>
}
...
export function startReminderScheduler(params: {
  ...
  intervalMs?: number
  // If true, align scheduler to minute boundaries and then run at `intervalMs`.
  ...
  const advanceOnce = async () => {
    ...
    try {
      ...
      for (const reminder of due) {
        ...
        if (!reminder.recurrence) continue // Data should be consistent; skip defensively.

        const maxAdvances = 24
        let nextDueAtMs = computeNextDueAtMs({
        ...
        while (nextDueAtMs <= nowMs && advances < maxAdvances) {
          nextDueAtMs = computeNextDueAtMs({
            dueAtMs: nextDueAtMs,
            timezone: reminder.timezone,
            recurrence: reminder.recurrence,
          })
